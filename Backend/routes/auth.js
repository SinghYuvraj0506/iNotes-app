...
router.post(
  ...
  async (req, res) => {
    ...
    try {
      ...
      if (user) {
        // if user already exists then send the response
        return res
          .status(400)
          .json({
            success,
      ...
      // try catch statement does something and if it has some error then the catch ststement something something on error
